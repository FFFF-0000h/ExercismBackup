...
def _translate_word(word: str) -> str:
    ...
    if _starts_with_vowel_sound(lower_word, vowel_sounds):
        translated_result = word + "ay"
    else:
        # Rule 2-4: Word starts with consonant(s)
        prefix = _get_consonant_prefix(lower_word, consonants)
...
def _starts_with_vowel_sound(word: str, vowel_sounds: tuple) -> bool:
    """Check if word starts with a vowel sound."""
    return word.startswith(vowel_sounds)


def _get_consonant_prefix(word: str, consonants: set) -> str:
    """
...
if __name__ == "__main__":
    ...
    for input_text, expected in test_cases:
        try:
            test_result = translate(input_text)
    ...
    while True:
        try:
            ...
            if user_input.lower() == "quit":
                break
            if user_input:
                print(f"Pig Latin: {translate(user_input)}")
        except ValueError as e:
            print(f"Error: {e}")
