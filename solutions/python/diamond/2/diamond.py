...
def rows(letter: str) -> list[str]:
    ...
    for row_index in range(max_idx + 1):
        current_letter = chr(ord("A") + row_index)
        line = [" "] * width
        left_pos = max_idx - row_index
        right_pos = max_idx + row_index

        line[left_pos] = current_letter
        if row_index != 0:                     # no second letter for the first row
