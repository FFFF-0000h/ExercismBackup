...
// localeConfig holds formatting rules for a specific locale.
...
// FormatLedger returns a formatted ledger string for the given entries.
...
// validateCurrency checks if the currency is supported and returns its symbol.
...
// validateLocale checks if the locale is supported and returns its config.
func validateLocale(locale string) (localeConfig, error) {
	config, ok := supportedLocales[locale]
	if !ok {
		return localeConfig{}, errors.New("invalid locale")
	}
	return config, nil
}

// sortEntries sorts entries by date, then description, then change.
...
// writeHeader writes the column headers to the builder.
func writeHeader(builder *strings.Builder, config localeConfig) {
	fmt.Fprintf(
		builder,
		"%-10s | %-25s | %-13s\n",
		config.headers[0],
...
// formatEntryLine formats a single entry as a ledger line.
func formatEntryLine(entry Entry, symbol string, config localeConfig) (string, error) {
...
// formatDate parses and reformats a date string according to the given layout.
...
// truncateDescription truncates a description to at most 25 characters.
...
// formatAmount formats a monetary amount with currency symbol and locale-specific formatting.
func formatAmount(change int, symbol string, config localeConfig) string {
...
// formatWithThousands formats an integer with thousands separators.
